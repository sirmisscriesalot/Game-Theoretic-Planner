...
class OptProb:
    ...
    def get_collision_constraint(self, theta_im, theta_j):
        ...
        for k in range(1, N + 1):
            ...
            cons_lhs.append(ca.mtimes(ca.transpose(b), theta_j.X[:, k] - self.X_[:, k]))
    ...
    def get_wall_constraint(self, theta_im):
        ...
        cons_lhs = []
        for k in range(1, N + 1):
            cons_lhs.append(ca.mtimes(ca.transpose(theta_im.normal[:, k]), self.X_[:, k] - theta_im.tau[:, k]))
    ...
    def get_objective(self, theta_im, theta_il, theta_j):
        ...
        simga_den = 1 - CURV*ca.mtimes(ca.transpose(theta_im.X[:, N] - theta_im.tau[:, N]), theta_im.normal[:, N])
        sigma = sigma_num/simga_den
        obj = ca.mtimes(ca.transpose(sigma), self.X_[:, N])
        ...
        for k in range(N + 1):
            ...
            if self.blocking_factor != 0:
                obj = obj + self.blocking_factor*theta_j.mu[k - 1]*ca.mtimes(ca.transpose(b), self.X_[:, k])
...
def get_strategy(theta_il, theta_j, blocking_factor):
    theta_im = copy.deepcopy(theta_il)
    for m in range(M):
...
plt.plot(theta_i.X[0][:N-10], theta_i.X[1][:N-10], 'r')

plt.plot(P_j0[0], P_j0[1], 'go')
plt.plot(theta_j.X[0][:N-10], theta_j.X[1][:N-10], 'g')
